...
document.getElementById('clickMe').onclick = function() {
    var all = document.getElementById("comand").value;
    ...
    for (var i = 0; i < split.length; i++) {
        if (split[i] == "forward") {
            turtle.forward(split[i+1]);
        } 
        if (split[i] == "backward") {
            turtle.backward(split[i+1]);
        } 
        if (split[i] == "left") {
            turtle.left(split[i+1]);
        } 
        if (split[i] == "right") {
            turtle.right(split[i+1]);
        } 
        if (split[i] == "penDown") {
            turtle.penDown = true;
        } 
